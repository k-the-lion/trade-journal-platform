...
export function TopstepXSyncPanel({
  ...
  const [topstepAccounts, setTopstepAccounts] = useState<TopstepXAccountOption[]>([]);
  ...
  async function handleVerify() {
    ...
    try {
      const { accounts } = await verifyTopstepXCredentials(username, apiKey);
      setTopstepAccounts(accounts);
      if (accounts.length === 1) {
        setExternalAccountId(String(accounts[0]!.id));
      }
      setMessage(`Found ${accounts.length} TopstepX account${accounts.length === 1 ? "" : "s"}.`);
    } catch (err) {
  ...
  async function handleConnect() {
    if (!externalAccountId || !tradingAccountId) {
      setError("Select both a TopstepX account and a journal account");
      return;
    }
    const selected = topstepAccounts.find((a) => String(a.id) === externalAccountId);
    if (!selected) {
      setError("Select a TopstepX account");
      return;
    }
    ...
    setMessage(null);
    try {
      const connection = await connectTopstepX({
        username,
        apiKey,
  ...
  async function handleSync(connectionId: string) {
    ...
    setMessage(null);
    try {
      const result = await syncTopstepXConnection(connectionId);
      setConnections((prev) =>
        prev.map((c) =>
          c.id === connectionId
            ? {
  ...
  async function handleDisconnect(connectionId: string) {
    if (!confirm("Disconnect this TopstepX account? Your imported trades will stay in the journal.")) {
      return;
    }
    setError(null);
    try {
  ...
  const selectedTopstepName = topstepAccounts.find(
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      <div className="card p-6 space-y-5">
        <div>
          <h2 className="font-medium">Connect TopstepX</h2>
          <p className="text-xs text-muted mt-1">
            Requires a{" "}
            <a
              href="https://help.topstepx.com/settings/api"
              target="_blank"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          <div>
            <label className="label">TopstepX username</label>
            <input
              className="input"
              value={username}
        ...
        <button
          ...
          disabled={verifying || !username.trim() || !apiKey.trim()}
        ...
        {topstepAccounts.length > 0 && (
          <div className="space-y-4 pt-2 border-t border-border/60">
            <div>
              <label className="label">TopstepX account</label>
              <select
                className="input"
                value={externalAccountId}
                onChange={(e) => setExternalAccountId(e.target.value)}
              >
                <option value="">Select combine / funded account…</option>
                {topstepAccounts.map((a) => (
            ...
            <div>
              ...
              />
              <p className="text-xs text-muted mt-1">
                Leave empty to pull up to 12 months of history on the first sync.
              </p>
            </div>
            ...
            >
              {connecting
                ? "Connecting…"
                : `Connect${selectedTopstepName ? ` ${selectedTopstepName}` : ""}`}
            </button>
          </div>
        )}
      </div>
